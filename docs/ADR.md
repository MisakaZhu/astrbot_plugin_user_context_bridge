...
> （历史记录：本节"裸 `__mode_user__` 一律隔离"的表述未按输入版本区分，
> 已被 ADR-017 修订——v1 来源按字面值保留，仅 v2 来源歧义才隔离。）
...
## ADR-017：Y1–Y4 返工修订（2026-09-20，基于 Codex 对 4d73c7a 的独立复验）

### A17-1 迁移解码按输入版本（修订 A16-1）

- 旧库键改写必须以**来源 schema 版本**解码后再编码，不按目标版本键的前缀猜测。v1（0.6.0）来源的 scope 段一律是原始人格名字面值：`maid`、`p:maid`、`u:`、`q:foo`、`__mode_user__` 等全部按字面值加 `p:` 前缀保留（`p:maid`、`p:__mode_user__`），不做隔离；仅来源确定为 v2 的 `__mode_user__`（可能承载 user 模式记录）才改写为 `q:` 隔离并按安全方向追加基础保护。`meta` 记录 `__migration__`/`source_version`，membership 同源。
- 两遍键改写（先写临时键再落最终键）消除同库 `maid` 与 `p:maid` 并存时的 UNIQUE 中间态碰撞。

### A17-2 首次命令身份登记与命令失败协议（扩展 A16-2；Z2 修订）

- 除 begin_turn 首次参与外，`/uctx off`/`/uctx on` 等产生持久化身份状态的命令入口同样登记当前生效模式：未记录 → 登记不推进代次；同模式 → 幂等；命令本身不推进代次，显式配置切换由 initialize 对账恰好推进一次。区分 v1 升级登记、新身份首次命令参与、已生效身份配置切换三类。
- **命令保存/登记采用"模式登记先行"顺序（Z2 修订）**：先 `apply_scope_mode` 再写 membership。登记失败（含真实 sqlite3.Error——`BEGIN IMMEDIATE` 锁冲突、语句约束失败，不只 LedgerError）即整条命令受控失败返回，此时退出/加入尚未写入，不产生"退出已保存而模式事实缺失"的不可恢复分裂（该分裂曾使登记失败后直接切另一模式 0→0 漏推进）。后一写（membership）失败只留下"已登记未退出"状态：登记本身不改代次、与 initialize 对账结果等价，重试自愈。
- **失败 on 不解除既有退出**：数据库占用等真实故障下，on 在登记阶段即受控返回，membership 的解除不执行，磁盘/内存/status/后继采集一致（退出生效、captured=0），去障后重试成功才解除。
- 命令文案与真实结果一致（"未生效+状态保持不变+可重试"），不泄漏内部路径。

### A17-3 备份与回滚事实（修订 A16-3 文档口径）

- 备份名为 `backups/pre-migrate-v3-<时间戳>-<库名>`（W 轮文档误写 pre-migrate-v2）；方式为 SQLite backup API 整库一致性快照，不是 WAL checkpoint+复制主库。
- 回滚步骤已用真实 0.6.0（859f18e）代码验证：恢复 `pre-migrate-v3-*` 备份与 `membership.json.pre-scheme3.bak` 后旧代码可读可写、裸键退出命中；v3 前缀退出文件在旧代码下裸键不命中。直接换旧代码指回 v3 库的真实行为是**静默键分裂**（可打开、旧记录不可见、新写以裸键分叉），不是 W 轮文档声称的"列冲突报错"。
