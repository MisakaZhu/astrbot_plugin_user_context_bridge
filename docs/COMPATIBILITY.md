...
**不要**不恢复备份直接把旧代码指回 v3 库。在**含旧记录的迁移库副本**上实测（真实 0.6.0 代码，local_evidence/y4_rollback_verify.py 双版 verified_silent_key_split=true）：库可正常打开（建表为 `IF NOT EXISTS`，不因多余列失败），但旧代码按裸身份键读取——迁移后的旧问答（`p:` 前缀键，数据仍在库中）对它完全不可见（裸键读到 0 条），新写入以裸键落盘形成第二套并行历史（编码键 1 行 + 裸键 1 行并存）。这比报错更危险：不损坏文件、没有任何警告，但历史"消失"且数据分叉。该行为与 W 轮文档声称的"列冲突错误"不符，特此更正。

另须明确：第 2/3 步恢复的是**升级前快照**——升级到 0.7.0 之后新增的共享历史与退出状态变更不在备份内，恢复后不会自动带回；如需保留这部分增量，应在回滚前用 `tools/uctx_records.py` 导出存档。不在朋友真实库上做任何回滚试验。
